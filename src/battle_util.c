...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_FRISK:
            ...
            return effect; // Hinweis: Frisk wird nicht aufgezeichnet, wenn es nicht aktiviert wird.
        ...
        default:
            break;
        } // DIESE KLAMMER UND DER BREAK HABEN GEFEHLT!
        break;
...
{
    // Wenn Safeguard auf der Seite des Verteidigers nicht aktiv ist, gibt es keinen Schutz
    if (!(gSideStatuses[GetBattlerSide(battlerDef)] & SIDE_STATUS_SAFEGUARD))
        return FALSE;
    
    // Infiltrator ignoriert Safeguard komplett
    if (abilityAtk == ABILITY_INFILTRATOR)
        return FALSE;

    // Verbündete sind durch Safeguard geschützt (oder blockiert, je nach Sichtweise)
...
bool32 CanSetNonVolatileStatus(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum MoveEffect effect, enum ResultOption option)
{
    ...
    // Spezifische Checks pro Status-Effekt
    switch (effect)
    {
    ...
    case MOVE_EFFECT_FROSTBITE:
        ...
        {
            battleScript = BattleScript_AlreadyBurned; // Nutzt intern oft das gleiche Skript
        }
    ...
    // Allgemeine Checks für alle Statusveränderungen
...
bool32 CanFling(enum BattlerId battlerAtk, enum BattlerId battlerDef)
{
    ...
    if (item == ITEM_NONE
      ...
      || gBattleMons[battlerAtk].volatiles.embargo // Fix: battlerAtk statt battler
      // Nyxthra-Spezial: Verhindert das Schleudern von unendlichen TMs/HMs
      || (GetItemTMHMIndex(item) != 0 && GetItemImportance(item) == 1)
