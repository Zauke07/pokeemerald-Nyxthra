...
static void OpenMoveRelearnerFromPartyMenu(u8 taskId, enum MoveRelearnerStates state);
static void Task_OpenMoveRelearnerFromPartyMenu(u8 taskId);
...
static void CursorCb_TakeItem(u8 taskId)
{
    ...
    {
    case TAKE_NO_ITEM: // Not holding item
        GetMonNickname(mon, gStringVar1);
        StringExpandPlaceholders(gStringVar4, gText_PkmnNotHolding);
        DisplayPartyMenuMessage(gStringVar4, TRUE);
        break;
...
static void OpenMoveRelearnerFromPartyMenu(u8 taskId, enum MoveRelearnerStates state)
{
    ...
    // Der aktuelle Move-Relearner kennt keinen RELEARN_MODE_PARTY_MENU mehr.
    // Deshalb schließen wir das Team-Menü selbst und öffnen den Relearner
    // direkt nach dem Fade. Beim Beenden geht es sauber zurück ins Feld.
    BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
    gTasks[taskId].func = Task_OpenMoveRelearnerFromPartyMenu;
}
...
{
    OpenMoveRelearnerFromPartyMenu(taskId, MOVE_RELEARNER_LEVEL_UP_MOVES);
}

static void CursorCb_ChangeEggMoves(u8 taskId)
{
    OpenMoveRelearnerFromPartyMenu(taskId, MOVE_RELEARNER_EGG_MOVES);
}

static void CursorCb_ChangeTMMoves(u8 taskId)
{
    OpenMoveRelearnerFromPartyMenu(taskId, MOVE_RELEARNER_TM_MOVES);
}

static void CursorCb_ChangeTutorMoves(u8 taskId)
{
    OpenMoveRelearnerFromPartyMenu(taskId, MOVE_RELEARNER_TUTOR_MOVES);
}
