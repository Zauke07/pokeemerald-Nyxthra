...
// DEBUG: Hilfsarray für Laufzeitwerte
EWRAM_DATA u16 sDebugStatusVals[8];
...
/*
// Testweise auskommentiert, weil 3 paletten zu viel
// Battler → OBJ-Palette für die Healthbox
// 0 = Spieler      → Palette 4 (wie früher)
// 1 = Gegner       → Palette 13
// 2 = Spieler-Partner → Palette 14
// 3 = Gegner-Partner  → Palette 15
static const u8 sHealthboxPalPerBattler[MAX_BATTLERS_COUNT] =
{
    4,   // battler 0
    13,  // battler 1
    14,  // battler 2
...
static void BI_AssignPerBattlerPalettes(u8 battler, u8 healthboxLeftSpriteId, u8 healthboxRightSpriteId)
{
    u8 palNum;
    ...
    if (palNum >= 13)
    {
        LoadPalette(gBattleInterface_BallStatusBarPal,
                    OBJ_PLTT_OFFSET + palNum * 16,
                    PLTT_SIZEOF(16));
    }

    // Nur die Healthbox-Sprites (links + rechts) auf diese Palette legen.
    gSprites[healthboxLeftSpriteId].oam.paletteNum  = palNum;
    gSprites[healthboxRightSpriteId].oam.paletteNum = palNum;
}
*/
...
void UpdateHpTextInHealthbox(u32 healthboxSpriteId, u32 maxOrCurrent, s16 currHp, s16 maxHp)
{
    ...
    // --- Nyxthra Fix: Geister-Partner im Doppelkampf ignorieren ---
    if (IsDoubleBattle() && gPlayerPartyCount == 1 && GetBattlerPosition(battler) == B_POSITION_PLAYER_RIGHT)
        return;
...
void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    // Sprite-Daten temporär sichern (RHH Standard)
...
// ============================================================================
// STATUS-ICON: 1x Palette (Palette 4) mit 4x Slots (12..15)
// Diese Logik setzt voraus, dass die GFX-Assets (status.4bpp) für jeden Battler
// auf den korrekten Farb-Index (12, 13, 14, 15) verweisen.
// ============================================================================
static void UpdateStatusIconInHealthbox(u8 healthboxSpriteId)
{
    ...
    if (IsOnPlayerSide(battler))
        tileNumAdder = (GetBattlerCoordsIndex(battler) == BATTLE_COORDS_SINGLES) ? 0x1A : 0x12;
    else
        tileNumAdder = 0x11;

    // --- Status Auswahl mit deinem Paletten-Fix ---
    if (status & STATUS1_SLEEP) { statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_SLP_BATTLER0, battler)); statusPalId = PAL_STATUS_SLP; }
    else if (status & STATUS1_PSN_ANY) { statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_PSN_BATTLER0, battler)); statusPalId = PAL_STATUS_PSN; }
    else if (status & STATUS1_BURN) { statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_BRN_BATTLER0, battler)); statusPalId = PAL_STATUS_BRN; }
    else if (status & STATUS1_FREEZE) { statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_FRZ_BATTLER0, battler)); statusPalId = PAL_STATUS_FRZ; }
    else if (status & STATUS1_FROSTBITE) { statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_FRB_BATTLER0, battler)); statusPalId = PAL_STATUS_FRZ; }
    else if (status & STATUS1_PARALYSIS) { statusGfxPtr = GetHealthboxElementGfxPtr(GetStatusIconForBattlerId(HEALTHBOX_GFX_STATUS_PRZ_BATTLER0, battler)); statusPalId = PAL_STATUS_PAR; }
    else
    {
        // Kein Status: Icons löschen
        ...
        dstIndex = gSprites[healthboxSpriteId].oam.paletteNum * 16 + (12 + battler);
        FillPalette(gBattleInterface_BallStatusBarPal[12], OBJ_PLTT_OFFSET + dstIndex, PLTT_SIZEOF(1));
        CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_OFFSET + dstIndex], (u16 *)OBJ_PLTT + dstIndex, PLTT_SIZEOF(1));

        TryAddPokeballIconToHealthbox(healthboxSpriteId, TRUE);
        return;
    }

    // Status vorhanden: Farbe in Battler-Index (12-15) schreiben
    dstIndex = gSprites[healthboxSpriteId].oam.paletteNum * 16 + (12 + battler);
    FillPalette(sStatusIconColors[statusPalId], OBJ_PLTT_OFFSET + dstIndex, PLTT_SIZEOF(1));
    CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_OFFSET + dstIndex], (u16 *)OBJ_PLTT + dstIndex, PLTT_SIZEOF(1));
...
static void MoveBattleBarGraphically(enum BattlerId battler, u8 whichBar)
{
    ...
    {
    case HEALTH_BAR:
        // Wir ignorieren den kaputten GEN_5 Block und nutzen direkt die Pixel-Breite!
        ...
        // B_HEALTHBAR_PIXELS ist die Gesamtlänge der Leiste (meist 48 Pixel)
        if (currValue > (B_HEALTHBAR_PIXELS * 50 / 100)) // Mehr als 50% (24 Pixel)
            barElementId = HEALTHBOX_GFX_HP_BAR_GREEN;
        else if (currValue > (B_HEALTHBAR_PIXELS * 20 / 100)) // Mehr als 20% (10 Pixel)
            barElementId = HEALTHBOX_GFX_HP_BAR_YELLOW;
        else
            barElementId = HEALTHBOX_GFX_HP_BAR_RED; // 20% oder weniger
...
// Deine Text-Printer Hilfsfunktionen
static u8 *AddTextPrinterAndCreateWindowOnHealthboxWithFont(const u8 *str, u32 x, u32 y, u32 bgColor, u32 fgColor, u32 shadowColor, u32 *windowId, u32 fontId)
{
    u16 winId;
    u8 color[3];
    ...
    color[0] = bgColor; color[1] = fgColor; color[2] = shadowColor;
...
{
    return AddTextPrinterAndCreateWindowOnHealthboxWithFont(str, x, y, bgColor, 1, 3, windowId, FONT_SMALL);
}

static u8 *AddTextPrinterAndCreateWindowOnHealthboxToFit(const u8 *str, u32 x, u32 y, u32 bgColor, u32 fgColor, u32 shadowColor, u32 *windowId, u32 width)
{
    u32 fontId = GetFontIdToFit(str, FONT_SMALL, 0, width);
    return AddTextPrinterAndCreateWindowOnHealthboxWithFont(str, x, y, bgColor, fgColor, shadowColor, windowId, fontId);
}
