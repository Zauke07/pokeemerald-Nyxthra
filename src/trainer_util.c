...
{
    u32 data;
    // Nyxthra: species randomizer support - this used to live inline in
    // battle_main.c's CreateNPCTrainerPartyFromTrainer before that function
    // was replaced by this shared generator (also used for partner trainers).
    // Gender/personality generation and CreateMon() itself all need to use
    // the randomized species; the ability slot lookup below deliberately
    // does not, since an ability index only makes sense for the species it
    // was configured for.
    u32 species = Randomizer_GetSpecies(trainerMon->species, RANDOMIZER_MODE_TRAINER);
    ...
    if (trainerMon->gender == TRAINER_MON_RANDOM_GENDER)
        ...
        genderValue = GeneratePersonalityForGender(MON_MALE, species);
    else if (trainerMon->gender == TRAINER_MON_FEMALE)
        genderValue = GeneratePersonalityForGender(MON_FEMALE, species);
    else
    ...
    if (trainerMon->heldItem != ITEM_NONE)
    {
        u32 heldItem = Randomizer_GetItem(trainerMon->heldItem, RANDOMIZER_MODE_TRAINER);
    ...
    // Ability slot indices only make sense for the species they were
    // configured for, so skip explicit ability assignment if the
    // randomizer swapped the species (falls through to the random-ability
    // logic below instead, same as the pre-refactor behavior).
    bool32 abilitySet = FALSE;
    if (trainerMon->ability && species == trainerMon->species)
    {
