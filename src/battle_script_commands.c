...
static void Cmd_moveend(void)
{
    ...
    // Wenn DoMoveEnd fertig ist (MOVEEND_COUNT erreicht), geht es zum nächsten Befehl
...
static u32 ChangeStatBuffs(enum BattlerId battler, s8 statValue, enum Stat statId, union StatChangeFlags flags, u32 stats, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        if (gSideTimers[GetBattlerSide(battler)].mistTimer
            && !flags.certain 
            && gCurrentMove != MOVE_CURSE  // Deine präzise Attacken-Prüfung
            && !(battler == gBattlerTarget 
                 && GetBattlerAbility(gBattlerAttacker) == ABILITY_INFILTRATOR 
                 && !IsBattlerAlly(gBattlerAttacker, battler))) // Dein Allianz-Check bleibt!
        {
            if (flags.allowPtr)
            {
