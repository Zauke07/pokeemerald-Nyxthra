...
extern bool32 IsSafeguardProtected(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk);
...
static void Cmd_getexp(void)
{
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsMonShiny(&gParties[B_TRAINER_OPPONENT_A][0]) || 
                    (WILD_DOUBLE_BATTLE && IsMonShiny(&gParties[B_TRAINER_OPPONENT_A][1])))
                {
                ...
                if (IsValidForBattle(&gParties[B_TRAINER_PLAYER][*expMonId]))
                {
                    ...
                    PREPARE_MON_NICK_WITH_PREFIX_BUFFER(gBattleTextBuff1, expGetterBattlerId, *expMonId);
    ...
    case 4: // lvl up if necessary
        ...
        {
            u8 monId = *expMonId;
            u32 exp = GetMonData(&gParties[B_TRAINER_PLAYER][monId], MON_DATA_EXP);
            (void)exp;
            if (gBattleResources->bufferB[0][0] == CONTROLLER_TWORETURNVALUES && gBattleResources->bufferB[0][1] == RET_VALUE_LEVELED_UP)
            {
                ...
                AdjustFriendship(&gParties[B_TRAINER_PLAYER][monId], FRIENDSHIP_EVENT_GROW_LEVEL);
...
static void Cmd_moveend(void)
{
    ...
    // Sicherheits-Checks der Expansion
    assertf(gBattlerAttacker < MAX_BATTLERS_COUNT, "invalid gBattlerAttacker: %d\nmove: %S", gBattlerAttacker, GetMoveName(gCurrentMove));
    assertf(gBattlerTarget < MAX_BATTLERS_COUNT, "invalid gBattlerTarget: %d\nmove: %S", gBattlerTarget, GetMoveName(gCurrentMove));

    // Die Magie passiert jetzt hier drin:
...
static void Cmd_givecaughtmon(void)
{
    ...
    case GIVECAUGHTMON_GIVE_AND_SHOW_MSG:
    {
        ...
        {
            enum Item lostItem = gBattleStruct->itemLost[B_TRAINER_OPPONENT_A][gBattlerPartyIndexes[caughtBattler]].originalItem;
...
static void Cmd_trysetcaughtmondexflags(void)
{
    ...
    enum BattlerId caughtBattler = GetCatchingBattler();
    struct Pokemon *caughtMon = GetBattlerMon(caughtBattler);
    enum Species species = GetCaughtMonOriginalSpecies(caughtBattler);
