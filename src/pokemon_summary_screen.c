    ...
    /*0x70*/ struct PokeSummary
        {
            ...
            u16 spdef; // 0x2A
            u16 speed; // 0x2C
            enum Item item; // 0x2E (Expansion Enum)
            u16 friendship : 8; // 0x30 (Nyxthra Affection System)
            u16 affection : 8;  // 0x30 (Nyxthra Affection System)
            u8 OTGender; // 0x32
            u8 nature; // 0x33
            u8 ppBonuses; // 0x34
            u8 sanity; // 0x35
            u8 OTName[17]; // 0x36
            u32 OTID; // 0x48
            u8 OTStyle;  // 0x49 (Wichtig für deine 20 Styles)
            enum Type teraType; // (Expansion Feature)
            u8 mintNature; // (Expansion Feature)
        } summary;
...
{
    [MOVE_RELEARNER_LEVEL_UP_MOVES] = COMPOUND_STRING("{START_BUTTON} ERLERNEN LEVEL"),
    [MOVE_RELEARNER_EGG_MOVES] =      COMPOUND_STRING("{START_BUTTON} ERLERNEN EI"),
    [MOVE_RELEARNER_TM_MOVES] =       COMPOUND_STRING("{START_BUTTON} ERLERNEN TM"),
    [MOVE_RELEARNER_TUTOR_MOVES] =    COMPOUND_STRING("{START_BUTTON} ERLERNEN TUTOR"),
};
